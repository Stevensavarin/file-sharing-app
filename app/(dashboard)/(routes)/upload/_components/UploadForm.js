...
function UploadForm({uploadBtnClick,progress}) {
    ...
    items-center justify-center w-full h-64 border-2
     ...
     rounded-lg cursor-pointer
      bg-green-50 dark:hover:bg-green-800
       dark:bg-green-700 hover:bg-green-100 dark:border-green-600 dark:hover:border-green-500 dark:hover:bg-green-600">
        ...
        justify-center pt-5 pb-6">
                        ...
                        <p className="mb-2 text-lg md:text-2xl text-black-500
             dark:text-black-400">
                            <span className="font-semibold">
                                Click to upload </span>
                            or <strong className='text-primary'>
                                drag</strong> and <strong className='text-primary'>drop</strong></p>
                        <p className="text-xs text-black-500 dark:text-black-400">*Only SVG, PNG, JPG or GIF are allowed.* ( Max Size : 2MB)</p>
                    </div>
           ...
           {progress>0? <ProgressBar progress={progress} />:
            <button disabled={!file} className='p-2 bg-primary text-black
